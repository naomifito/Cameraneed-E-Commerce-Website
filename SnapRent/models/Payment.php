...
class Payment {
    ...
    public static function recordPayment($orderId, $amount, $paymentMethod, $transactionId, $proofOfPayment) {
        try {
            ...
            // lastInsertId() may not work reliably after CALL in MySQL.
            $paymentId = $conn->lastInsertId();
            if ($paymentId) {
                return (int)$paymentId;
            }

            $stmtFind = $conn->prepare("SELECT payment_id FROM payment WHERE order_id = ? ORDER BY created_at DESC, payment_id DESC LIMIT 1");
